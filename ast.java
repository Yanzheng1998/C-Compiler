...
class FormalsListNode extends ASTnode {
    ...
    public List<Type> nameAnalysis(SymTable symTab) {
        ...
        int curr = myFormals.size() * 4;
        for (FormalDeclNode node : myFormals) {
            Sym sym = node.nameAnalysis(symTab);
            if (sym == null) continue;
            typeList.add(sym.getType());
            //set offset for every sym
            //every sym size is 4, so -4 after every sym
            sym.setOffset(curr);
            curr = curr - 4;
        }
        return typeList;
...
class FnBodyNode extends ASTnode {
     ...
     */
    public void nameAnalysis(SymTable symTab, int curr) {
        symTab.offset = -8;
        myDeclList.nameAnalysis(symTab);
        myStmtList.nameAnalysis(symTab);
        local_size = - 8 - symTab.offset;
    ...
    public void codeGen() {
        if(this.local_size > 0)
            Codegen.generate("subu", "$sp", "$sp", this.local_size);
        myStmtList.codeGen();
    }
    ...
    public void codeGen() {
        for(StmtNode node : myStmts) {
            if(node instanceof ReturnStmtNode) {
                ((ReturnStmtNode)node).codeGen("return");
            }
            else {
    ...
    public Sym nameAnalysis(SymTable symTab, SymTable globalTab) {
        ...
        if (!badDecl) {  // insert into symbol table
            try {
                ...
                if(symTab.size() == 1) sym.setGlobal();
...
class FnDeclNode extends DeclNode {
    ...
    public Sym nameAnalysis(SymTable symTab) {
        ...
        myBody.nameAnalysis(symTab, typeList.size()*(-4)); // process the function body
    ...
    public void codeGen() {
        ...
        Codegen.generate("addu", Codegen.FP, Codegen.SP, Integer.toString((myFormalsList.length() * 4) + 8));


        // handle locals?
        Codegen.sta.push("_"+myId.name()+"_Exit");
        ...
        // TODO size not sure
        Codegen.generateIndexed("lw", Codegen.RA, Codegen.FP, -4*myFormalsList.length());
        Codegen.generate("move", Codegen.T0, Codegen.FP);
        Codegen.generateIndexed("lw", Codegen.FP, Codegen.FP, -4*myFormalsList.length() - 4);
        Codegen.generate("move", Codegen.SP, Codegen.T0);

        if(myId.name().equals("main")){
            Codegen.generate("li", Codegen.V0, Integer.toString(10));
...
class PostIncStmtNode extends StmtNode {
    ...
    public void codeGen() {
        ...
        if(myExp instanceof IdNode) {
            ...
            if (sym.getGlobal()) {
                Codegen.generate("sw", Codegen.T0, "_" + temp.name());
            } else {
...
class WriteStmtNode extends StmtNode {
    ...
    public void typeCheck(Type retType) {
        ...
        if(type.isStringType()) this.isString = true;
    ...
    public void codeGen() {
        int syscall=1;
        if(this.isString) syscall = 4;

            myExp.codeGen();
            Codegen.genPop(Codegen.A0);
            Codegen.generate("li", Codegen.V0, Integer.toString(syscall));
            Codegen.generate("syscall");
    }
...
class IfStmtNode extends StmtNode {
    ...
    public void codeGen() {
        String ifTrue = Codegen.nextLabel();
            myExp.codeGen();

            Codegen.genPop(Codegen.T0);
            Codegen.generate("beq", Codegen.T0, "0", ifTrue);

            //myDeclList.codeGen();
            myStmtList.codeGen(ifTrue);

            Codegen.genLabel(ifTrue);
    }
...
class IfElseStmtNode extends StmtNode {
    ...
    public void codeGen() {
            ...
            myThenStmtList.codeGen(ifTrue);
            Codegen.generate("b", done);

            Codegen.genLabel(ifTrue);
            //myElseDeclList.codeGen();
            myElseStmtList.codeGen(ifTrue);
...
class ReturnStmtNode extends StmtNode {
    ...
    public void codeGen(String nothing) {
        if(myExp != null){
            myExp.codeGen();
            Codegen.genPop(Codegen.V0);
        }
        Codegen.generate("b", (String)Codegen.sta.pop());
    }
...
class AssignNode extends ExpNode {
    ...
    public void codeGen() {
        ...
        if (myLhs instanceof IdNode) {
            Sym s = ((IdNode)myLhs).sym();
            if (s.getGlobal()) {
                Codegen.generate("sw", Codegen.T0, "_" + ((IdNode) myLhs).name());
            } else {
                Codegen.generateIndexed("sw", Codegen.T0, Codegen.FP, s.getOffset());
            }
        }
        Codegen.genPop(Codegen.T0);
...
class AndNode extends LogicalExpNode {
    ...
    public void codeGen() {
        ...
        Codegen.generate("beq", "$t0", "0", shortCircuit);
...
class OrNode extends LogicalExpNode {
    ...
    public void codeGen() {
        ...
        Codegen.generate("beq", "$t0", "1", shortCircuit);
...
class EqualsNode extends EqualityExpNode {
    ...
    public void codeGen() {
        ...
        if(myExp1 instanceof StringLitNode && myExp2 instanceof StringLitNode) {
            String se1 = ((StringLitNode)myExp1).strVal();
              String se2 = ((StringLitNode)myExp2).strVal();
              if(se1.equals(se2)){
